...
def fcn_model(inputs, num_classes):
    ...
    encoded_layer = encoder_block(inputs, filters=32, strides=2)
    print(' : encoded_layer:', encoded_layer)
    # TODO: 増やす

    # TODO Add 1x1 Convolution layer using conv2d_batchnorm().
    # 1x1 レイヤ
    layer_1x1 = conv2d_batchnorm(encoded_layer, 64, kernel_size=1, strides=1)
    print(' : encoded_layer -> layer_1x1:', layer_1x1)
    
    # TODO: Add the same number of Decoder Blocks as the number of Encoder Blocks
    # デコーダー
    decoded_layer = decoder_block(layer_1x1, inputs, 32)
    print(' : layer_1x1 -> decoded_layer:', decoded_layer)
    
    # The function returns the output layer of your model. "x" is the final layer obtained from the last decoder_block()
    return keras.layers.Conv2D(num_classes, 3, activation='softmax', padding='same')(decoded_layer)
